import { success, error as popUpError, willDelete } from '../../services/alertService';
...
export const showDeleteCategory = async (id, setIsLoading) => {
    ...
    if (response) {
        ...
        ok
            ?
            success({ text: "Categoría eliminada correctamente" }) && setIsLoading(false)
            :
            popUpError({ text: `${error.message}` }) && setIsLoading(false)
    }
}
